...
export const generatePackagePDF = (pkg: Package) => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('Travelzada DMC - Professional Travel Operations Report', CONFIG.MARGIN, CONFIG.PAGE_HEIGHT - 10);
    doc.text(`Page ${i} of ${pageCount}`, CONFIG.PAGE_WIDTH - CONFIG.MARGIN, CONFIG.PAGE_HEIGHT - 10, { align: 'right' });
  }

  // --- 5. EXPORT ---
  const dateStr = new Date().toISOString().split('T')[0];
  const filename = `travelzada-${sanitizeFilename(pkg.name)}-${dateStr}.pdf`;
  doc.save(filename);
};
